...
{
    const QString headlessPath = qEnvironmentVariable("SEQEYES_HEADLESS_MARKER_FILE");
    const QString lifecyclePath = qEnvironmentVariable("SEQEYES_LIFECYCLE_MARKER_FILE");

    QStringList paths;
    if (!headlessPath.isEmpty())
        paths.push_back(headlessPath);
    if (!lifecyclePath.isEmpty() && lifecyclePath != headlessPath)
        paths.push_back(lifecyclePath);
...
static void emitLifecyclePhase(const char* phase, const QString& detail = QString())
{
    const QString lifecyclePath = qEnvironmentVariable("SEQEYES_LIFECYCLE_MARKER_FILE");
    const bool enabled = qEnvironmentVariableIsSet("SEQEYES_LIFECYCLE_MARKERS") || !lifecyclePath.isEmpty();
    if (!enabled)
        return;

    const qint64 pid = QCoreApplication::applicationPid();
    QString line = QString("[LIFECYCLE] pid=%1 phase=%2")
                       .arg(pid)
                       .arg(QString::fromLatin1(phase));
    if (!detail.isEmpty())
        line += QString(" detail=%1").arg(detail);
...
static void emitHeadlessPhase(bool headless, const char* phase, const QString& filePath = QString())
{
    if (!headless)
        return;
    ...
    if (filePath.isEmpty())
    {
        line = QString("[HEADLESS_PHASE] pid=%1 phase=%2")
                   .arg(pid)
                   .arg(QString::fromLatin1(phase));
    }
    else
    {
        ...
        line = QString("[HEADLESS_PHASE] pid=%1 phase=%2 file=%3")
                   .arg(pid)
                   .arg(QString::fromLatin1(phase))
                   .arg(fileName);
    }
...
int main(int argc, char *argv[])
{
    ...
    emitLifecyclePhase("qt_message_handler_installed");
    ...
    int exitCode = 0;
    {
        ...
        MainWindow window;
        emitHeadlessPhase(headless, "after_mainwindow_construct");
        emitLifecyclePhase("after_mainwindow_construct");
        ...
        if (!fileToOpen.isEmpty()) {
            ...
            emitHeadlessPhase(headless, "before_open_file", fileToOpen);
            window.openFileFromCommandLine(fileToOpen);
            emitHeadlessPhase(headless, "after_open_file", fileToOpen);
            ...
            if (parser.isSet("capture-snapshots")) {
                ...
                emitHeadlessPhase(headless, "capture_snapshots_start", fileToOpen);
                ...
                emitHeadlessPhase(headless, "exit_after_load_return", fileToOpen);
                emitLifecyclePhase("exit_after_load_return", QFileInfo(fileToOpen).fileName());
                exitCode = 0;
            }
        }
        ...
        if (exitCode == 0 && parser.isSet("automation")) {
            ...
            if (scen.isEmpty()) {
                ...
                emitHeadlessPhase(headless, "automation_start", scen);
                emitLifecyclePhase("automation_start", QFileInfo(scen).fileName());
                exitCode = AutomationRunner::run(window, scen);
                emitHeadlessPhase(headless, "automation_done", scen);
                emitLifecyclePhase("automation_done", QString::number(exitCode));
            }
        }

        if (!parser.isSet("exit-after-load") && !parser.isSet("automation"))
        {
            emitLifecyclePhase("before_app_exec");
            exitCode = app.exec();
            emitLifecyclePhase("after_app_exec", QString::number(exitCode));
        }
